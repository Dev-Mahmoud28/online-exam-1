...
export class QuestionsComponent implements OnInit, OnDestroy{
  ...
  setBreadcrumb(labelDiploma:string, labelExam:string, idExam:string, idDiploma:string){
    this._breadcrumbService.setItems([{label:"Diplomas", url:"/home/diplomas"},{label:labelDiploma, url:`/home/exams/${idDiploma}`}, {label:`${labelExam} Questions`, url:`/home/questions/${idExam}`}])
  }
  ...
  getQuestions(){
    this._diplomasService.getQuestion(this.examId()).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next:(res)=>{
  ...
  getExamById(){
    this._diplomasService.getExamById(this.examId()).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next:(res)=>{
        ...
        this.setBreadcrumb(res.payload.exam.diploma.title,this.title(),this.examId(),res.payload.exam.diplomaId);
      }
    })
  }
  ...
  submitAnswers(){
    ...
    this._diplomasService.submission(data).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next:(res)=>{
